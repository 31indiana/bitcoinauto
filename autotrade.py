...
while True:
    try:
        ...
        start_time = get_start_time("KRW-XRP")    #시작시간(오전9시로 설정)
        end_time = start_time + datetime.timedelta(days=1)   #끝나는 시간  오전9시 + 1일 = 다음날 오전 9시

        # 09:00:00 am < 현재 < 다음날 8:59:50 am
        if start_time < now < end_time - datetime.timedelta(seconds=10):  
            target_price = get_target_price("KRW-XRP", 0.5)   #새로운 전략의 경우 target_price를 구하는 코드를 수정하라.
            current_price = get_current_price("KRW-XRP")
            if target_price < current_price:
                krw = get_balance("KRW")
                if krw > 5000:  #최소주문금액 5000원 기준
                    upbit.buy_market_order("KRW-XRP", krw*0.9995)    #수수료 0.05% 
